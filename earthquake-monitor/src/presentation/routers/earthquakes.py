...
@router.post("/", response_model=EarthquakeResponseSchema, status_code=status.HTTP_201_CREATED)
async def create_earthquake(
    earthquake_data: CreateEarthquakeSchema,
    use_case: Annotated[CreateEarthquakeUseCase, Depends(get_create_earthquake_use_case)],
):
    try:
