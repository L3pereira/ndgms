...
class USGSService:
    ...
    def _parse_earthquake_feature(self, feature: dict) -> Earthquake | None:
        ...
        try:
            ...
            # Create earthquake entity with all required data
            # Note: USGS ID is at the root level of the feature, not in properties
            earthquake = Earthquake(
                location=location,
                magnitude=magnitude,
                occurred_at=occurred_at,
                source="USGS",
                external_id=feature.get("id"),  # ID is at root level
                raw_data=json.dumps(feature),
                title=properties.get(
                    "title"
                ),  # Extract title for user-friendly display
            )
